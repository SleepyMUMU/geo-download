...
- 2026-09-26 14:20 确认旧进程消失后清除了 PID `12832` 的陈旧锁，已在 Agent 前台以原文件范围、原 session ID 和 `jobs/session-input.txt` 启动**唯一**主流水线 PID `10228`；`jobs/pipeline.lock` 实测写 `10228`，任务正下载剩余地区，D 盘约 682 GiB 空闲。监控自动化 `automation` 已恢复为每小时运行，健康时静默。接手时重新核进程和锁；若仍运行，只监控，不启动第二条。
...
3. 当前主流水线已在运行；只监控。若中断，先确认旧进程和上传均不存在，再检查锁、未完成中间文件，以原范围、原 session ID `1790129428-xj43bb`、`jobs/session-input.txt` 在 Agent 前台安全续跑。单县失败保留错误并继续；每县新成果都需实际看全部 `comparison*.png`、记录 `visual_review.json` 后才可上传。按安全条件清理缓存。
