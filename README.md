...
- GDAL 压制使用 16 个处理线程、1024 MiB 重投影内存和 1024 MiB 块缓存；这些设置只对新启动的流水线进程生效。正在处理的地区不会在运行中切换线程数。
- 拼接使用 GDAL GTI 空间索引，保留 `mosaic_driver: vrt` 回退选项。焉耆县和博湖县现有瓦片的小样对照逐像素一致；大县提速幅度以完整运行记录为准。
...
已下载的小地区可用独立处理支路并行压制，例如 `python parallel_small.py 652826 652829`。该支路使用 `jobs/xinjiang-small-parallel/` 下独立的状态、工作目录与输出目录，只运行一条支路，不占用或改写主流水线的锁和状态表。结果必须先完成本地质量、视觉、哈希及夸克云端核验，再在主流水线安全停机后并入原任务；并入前不得将主状态表标为完成。
...
下载失败、缺瓦片、损坏瓦片、投影错误或质量不合格均不得标记为完成。经用户最新指示，已完成且通过本地哈希、质量、视觉复核和夸克云端回执核验的地区，可自动清理其参考影像、拼接 VRT/GTI 索引和不再被任何未完成地区使用的缓存瓦片。本地 GeoTIFF、质量记录、对照图和上传回执始终保留。
